...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B1', 'ห้อง')
            ->setCellValue('C1', 'วันที่จอง')
			->setCellValue('D1', 'วัตถุประสงค์')
			->setCellValue('E1', 'ผู้จอง')
            ->setCellValue('F1', 'เลขาฯ อนุมัติ')
			->setCellValue('G1', 'ยกเลิก');
...
while($ob2 = mysql_fetch_assoc($objQuery))
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $ob2["r_name"]);
	$objPHPExcel->getActiveSheet()->setCellValue('C' . $i, dateThai2($ob2['Dater']).' '.$ob2["time_in"].' - '.$ob2["time_out"]);
	$objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $ob2["title"]);
	$objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $ob2['uname'].' '.$ob2["Fname"].' โทร.'.$ob2["phone"]);
	$objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $cf_msgicon2_noicon[$ob2['confirm']]['title']);
	$objPHPExcel->getActiveSheet()->setCellValue('G' . $i, $ob2['uq_cancel']);
	$i++;
}
mysql_close();
